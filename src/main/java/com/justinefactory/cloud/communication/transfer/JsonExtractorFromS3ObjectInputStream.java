...
public class JsonExtractorFromS3ObjectInputStream implements ContentExtractorFromS3ObjectInputStream<JsonStorage> {

    private final Logger logger = LogManager.getLogger(this.getClass());

    @Override
    public JsonStorage extractContent(S3ObjectInputStream inputStream) throws AwsContentReadingException {
        if (inputStream == null) {
            throw new AwsContentReadingException("Trouble while processing inputStream - input was empty.");
        }
        try {
            return new JsonStorage(IOUtils.toString(inputStream));
        } catch (IOException e) {
